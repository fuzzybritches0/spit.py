...
class ConfigScreensMixIn:
    async def on_select_changed(self, event: Select.Changed) -> None:
        if event.control.id == "CustomSettingSelectAdd":
    ...
    async def mount_custom_setting_form(self, stype) -> None:
        ...
        await self.vscroll.mount(Input(id="NewSetting",
        ...
        if stype == "Select":
            ...
            await self.vscroll.mount(Input(id="NewSelectValues", validators=Validators,
                                           restrict=r"[0-9a-z_, ]*", valid_empty=False))
        await self.vscroll.mount(Button("Add", id=f"ButtonAddSetting"))

    async def mount_settings_add_custom(self, stype) -> None:
        while not self.vscroll.children[-1].id == "CustomSettingSelectAdd":
    ...
    async def edit_settings_add_custom(self) -> None:
        ...
        await self.vscroll.mount(Select.from_values(types, id="CustomSettingSelectAdd"))
    ...
    async def edit_settings_remove_custom(self) -> None:
        ...
        await self.vscroll.mount(Select.from_values(names, id="CustomSettingSelectRemove"))
        await self.vscroll.mount(Button("Remove", id=f"ButtonRemoveSetting"))
    ...
    async def edit_settings_screen(self) -> None:
        self.vscroll = VerticalScroll(id="EditSettings")
    ...
    async def select_config_screen(self) -> None:
        self.vscroll = VerticalScroll(id="SelectConfig")
        await self.dyn_container.mount(self.vscroll)
        ...
        for configs in self.config.configs:
            ...
            count+=1
        await self.vscroll.mount(OptionList(*Options, id="OptionList"))
