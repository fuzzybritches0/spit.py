...
out=$($H --path fixtures/shared-lf.txt --encoding utf-8 --show_line_numbers False)
check "t1-rc" 0 $?
expect_output "t1-name" "$out" "fixtures/shared-lf.txt"
expect_output "t1-content" "$out" "Line one"
expect_output "t1-content2" "$out" "Line three"

echo
echo "=== 2. show_line_numbers=True ==="
out=$($H --path fixtures/shared-lf.txt --encoding utf-8 --show_line_numbers True)
check "t2-rc" 0 $?
...
out=$($H --path fixtures/t03-twelve.txt --encoding utf-8 --show_line_numbers True)
check "t3-rc" 0 $?
...
out=$($H --path fixtures/t04-crlf.txt --encoding utf-8 --show_line_numbers True)
check "t4-rc" 0 $?
...
echo
echo "=== 5. Empty file ==="
out=$($H --path fixtures/t05-empty.txt --encoding utf-8 --show_line_numbers True)
check "t5-rc" 0 $?
expect_output "t5-name" "$out" "empty.txt"

echo
echo "=== 6. No trailing newline ==="
out=$($H --path fixtures/t06-no-trailing.txt --encoding utf-8 --show_line_numbers True)
check "t6-rc" 0 $?
...
out=$($H --path '["fixtures/shared-lf.txt","fixtures/t07-multi.txt"]' --encoding utf-8 --show_line_numbers False)
check "t7-rc" 0 $?
...
out=$($H --path '["fixtures/shared-lf.txt","fixtures/shared-no-such-file.txt"]' --encoding utf-8 --show_line_numbers False)
check "t8-rc" 0 $?
...
out=$($H --path fixtures/shared-no-such-file.txt --encoding utf-8 --show_line_numbers False)
check "t9-rc" 1 $?
expect_output "t9-msg" "$out" "ERROR"

echo
echo "=== 10. Blank lines preserved ==="
out=$($H --path fixtures/t10-blank-lines.txt --encoding utf-8 --show_line_numbers True)
check "t10-rc" 0 $?
