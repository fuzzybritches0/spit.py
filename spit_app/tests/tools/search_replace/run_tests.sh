...
cp fixtures/basic.txt work.txt
out=$($H --path work.txt --find old_word --replace new_word ${D_FALSE})
check "t1-rc" 0 $?
expect_output "t1-msg" "$out" "Replaced 3 of 3 match(es)"
if [ "$(count_occurrences work.txt old_word)" -eq 0 ]; then
  pass=$((pass+1))
else
...
cp fixtures/regex.txt work.txt
out=$($H --path work.txt --find "[0-9]+" --replace NUMBER \
  --use_regex True --max_replacements 0 --dry_run False)
check "t2-rc" 0 $?
expect_output "t2-msg" "$out" "Replaced 5 of 5 match(es)"
remaining=$(grep -oP '[0-9]' work.txt | wc -l)
...
cp fixtures/multiple.txt work.txt
out=$($H --path work.txt --find repeated --replace changed \
  --use_regex False --max_replacements 2 --dry_run False)
check "t3-rc" 0 $?
expect_output "t3-msg" "$out" "Replaced 2 of 5 match(es)"
rem=$(count_occurrences work.txt repeated)
chg=$(count_occurrences work.txt changed)
if [ "$rem" -eq 3 ]; then
  pass=$((pass+1))
else
...
cp fixtures/no_match.txt work.txt
md5b=$(md5 work.txt)
out=$($H --path work.txt --find nonexistent_string --replace X ${D_FALSE})
check "t4-rc" 0 $?
...
cp fixtures/dry_run.txt work.txt
md5b=$(md5 work.txt)
out=$($H --path work.txt --find unreplaced --replace replaced \
...
unchanged "t5" "$md5b" "$(md5 work.txt)"
...
cp fixtures/complex_regex.txt work.txt
out=$($H --path work.txt --find "\\b[A-Z][a-z]+\\b" --replace NAME \
  --use_regex True --max_replacements 0 --dry_run False)
check "t7-rc" 0 $?
expect_output "t7-msg" "$out" "Replaced 4 of 4 match(es)"
remaining=$(grep -oP '[A-Z][a-z]+' work.txt | wc -l)
if [ "$remaining" -eq 0 ]; then
  pass=$((pass+1))
else
...
cp fixtures/multiline.txt work.txt
out=$($H --path work.txt --find "line1\nline2" --replace "combined" \
  --use_regex True --max_replacements 0 --dry_run False)
check "t8-rc" 0 $?
expect_output "t8-msg" "$out" "Replaced 1 of 1 match(es)"
if grep -qF "combined" work.txt; then
  pass=$((pass+1))
else
...
out=$($H --path /tmp/nonexistent_sr_test_file --find x --replace y ${D_FALSE})
check "t9-rc" 1 $?
